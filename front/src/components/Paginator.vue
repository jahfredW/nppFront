<template>
    <v-container class="container has-text-centered px-10">
        ...
        <v-row>
            <FilmCard :test="filteredList" />
        </v-row>
        </div>
        <div class="btn-wrapper d-flex justify-content-around">
            <button type="button" class="btn bg-purple-lighteen-5" :disabled="currentPage === 1" @click="changePage(-1)"> Prev</button>
            <button type="button" class="btn" :disabled="currentPage === pageToDisabled" @click="changePage(1)">Next </button>
        </div>  
    </v-container>
</template>
...
export default{
    ...
    computed: {
        ...
        pageToDisabled(){
            return Math.floor(this.filmList.length / this.perPage);
        }
