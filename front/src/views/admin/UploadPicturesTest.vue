<template>
  <v-container class="container mt-10 w-75">
    <v-form>
      <v-container>
        <h1 class="h3 text-center mb-5">Créer une nouvelle session</h1>
        <v-text-field v-model="state.sessionName" placeholder="nom de la session" label="Nom de la session">
        </v-text-field>
        <div class="mb-5" v-if="$v.sessionName.$error">Le nom est obligatoire.</div>
        <div>
          <v-file-input show-size counter prepend-icon="mdi-camera" multiple placeholder="test" @change="onFileChange"
            label="photos à uploader"></v-file-input>
        </div>
        <div class="mb-5">
          <VueDatePicker v-model="sessionDate" :format="format" month-name-format="long" placeholder="date de la session">
          </VueDatePicker>
        </div>
        <div>
...
const format = (sessionDate) => {
  ...
  return `Sélection : ${day}/0${month}/${year}`;
}
  ...
  if (!$v.value.$error) {
    ...
    if (files.length > 0) {
      ...
      if (successUpload.length === 0) {
        ...
        formData.append('session_name', state.sessionName);
